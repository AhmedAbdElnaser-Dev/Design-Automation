...
public interface IApsOssService
{
    ...
    Task<BucketDetails> GetBucketDetailsAsync(string bucketKey, CancellationToken ct = default);
    Task<BucketDetails> CreateBucketAsync(string bucketKey, string? policyKey = null, string? region = null, CancellationToken ct = default);
    ...
    Task<ObjectDetails> UploadObjectAsync(string bucketKey, string objectKey, Stream content, long contentLength, string? contentType = null, CancellationToken ct = default);
